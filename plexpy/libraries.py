...
class Libraries(object):
    ...
    def __get_download_music(self, section_type, json_data):
        result = None
        return result

    def __get_download_shows(self, section_type, json_data):
        result = None
        return result

    def __get_download_movies(self, rating_key, json_data):
        logger.debug(u"__get_download_movies({0}, {1})".format(rating_key, json_data))
        monitor_db = database.MonitorDatabase()
        try:
            query = 'SELECT path,title,year,part,movie_id,confidence,is_processed,already_exists,is_upgrade FROM mm_movies'
            results = monitor_db.select(query)
        except Exception as e:
            logger.warn(u"PlexPy Libraries :: Unable to execute database query "
                "for __get_download_movies: {0}.".format(e))
            return None

        rows = []
        for row in results:
            if row['is_processed'] and row['title'] is not None:
                ratingKey = row['movie_id']
    ...
    def get_datatables_media_downloads(self, section_type=None, rating_key=None, kwargs=None):
        default_return = {
            'recordsFiltered': 0,
            'recordsTotal': 0,
            'draw': 0,
        ...
        if not section_type:
            logger.warn(u"PlexPy Libraries :: Datatable media downloads "
                        "called by invalid section_type provided.")
            default_return['error'] = "No section_type provided."
            return default_return
        ...
        # Get media downloads
        result = None
        if section_type in ('show', 'season', 'episode'):
            result = self.__get_download_shows(
                section_type, rating_key, json_data
            )
        elif section_type in ('artist', 'album', 'track'):
            result = self.__get_download_music(
                section_type, rating_key, json_data
            )
        elif section_type == 'movie':
            result = self.__get_download_movies(rating_key, json_data)
        else:
            return {
                'recordsFiltered': 0,
                'recordsTotal': 0,
                'draw': 0,
                'data': 'null',
                'error': 'Section type {0} not supported.'.format(section_type)
            }
